...
path_to_classified_folder ="/N/project/laidel_el_mcv/LAIDEL-MRI-tumor-id/results/kmeans-classified/"
...
def loop_body(lib_folder_path):
  ...
  if lib_folder_path.is_file() == False:
	...
	data_flair_path = lib_folder_path+"data_id"+"flair.nii"
   ...
   #load seg file and create derivatives
    data_seg_path = lib_folder_path+"data_id"+"seg.nii"
	...
	kmeans_path = path_to_kmeans_folder+"data_id"+".npy"
    ...
    performance_eval_lst.appened(performance_metrics) 
...
dct = {"names": np.asarray(data_order_ref),
	   "cluster_metrics": np.asarray(mean_var_list), 
	   "performance": np.asarray(performance_eval_lst}
